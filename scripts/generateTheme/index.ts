...
async function main(): Promise<void> {
  const { baseColor, neutralColor } = await args()

  if (!baseColor) {
    throw new Error(`baseColor is required`)
  }

  if (!neutralColor) {
    throw new Error(`neutralColor is required`)
  }
  ...
  fs.writeFileSync(path.join(__dirname, '../../themeColors.json'), JSON.stringify(themeColors, null, 2))

  // and baseColor and neutralColor to app.json
  fs.writeFileSync(path.join(__dirname, '../../app.json'), JSON.stringify({ ...app, baseColor, neutralColor }, null, 2))
}
